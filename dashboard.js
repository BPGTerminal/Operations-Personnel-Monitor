...
function CL(function CL(all){var h='<div class="card"><div class="card-hdr">COMMUNICATIONS LOG ('+all.length+')</div><div class="card-body"><div class="msg-log">';all.slice().reverse().slice(0,80).forEach(function(m){h+='<div class="mr"><span class="mt">'+esc((m.time||"").substring(0,19))+'</span><span class="ms">'+esc(m.sender||"")+(m.team?" ["+esc(m.team)+"]":"")+'</span><span class="mb">'+esc((m.body||"").substring(0,130))+(m.photo?" [photo]":"")+'</span></div>'});h+='</div></div></div>';return h}
