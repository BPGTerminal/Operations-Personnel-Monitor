...
function downloadWord(){
  var su=document.getElementById("unit").value||"ALL UNITS",df=document.getElementById("from").value||"beginning",dt=document.getElementById("to").value||"present";
  var c2=OPM.getConfig();
  var title=c2.eventName||"TERMINAL OPERATIONS AND MANAGEMENT REPORT";
  var loc=c2.location||"BROOKE'S POINT GRAND TERMINAL";

  // Gather content from the DOM, respecting include checkboxes
  var bodyHtml="";
  // Executive Summary
  var es=document.querySelector('.stats');
  if(es)bodyHtml+='<div style="margin-bottom:10px"><h3 style="color:#1a4d8f;font-size:14px">EXECUTIVE SUMMARY</h3>'+es.outerHTML+'</div>';

  // Unit Performance table
  var upCard=document.querySelector('.card:nth-of-type(2)');
  if(upCard&&upCard.querySelector('table'))bodyHtml+='<div style="margin-bottom:10px"><h3 style="color:#1a4d8f;font-size:14px">UNIT PERFORMANCE — '+esc(df)+' '+esc(dt)+'</h3>'+upCard.querySelector('table').outerHTML+'</div>';

  // Photo Reports — only included photos
  var prCard=document.querySelector('.photo-report-card');
  if(prCard){
    var inclState=getPhIncludes(su,df,dt);
    bodyHtml+='<div style="margin-bottom:10px"><h3 style="color:#1a4d8f;font-size:14px;page-break-before:always">PHOTO REPORTS</h3>';
    var pcEls=prCard.querySelectorAll('.photo-card');
    var anyIncluded=false;
    pcEls.forEach(function(pc){
      var chk=pc.querySelector('.photo-include-chk');
      if(chk&&!chk.checked)return;
      anyIncluded=true;
      bodyHtml+=pc.outerHTML;
    });
    if(!anyIncluded)bodyHtml+='<p style="color:#999">No photos selected for export.</p>';
    bodyHtml+='</div>';
  }

  // Inspection table
  var itCard=document.querySelector('.inspect-table-card');
  if(itCard)bodyHtml+='<div style="margin-bottom:10px;page-break-before:always"><h3 style="color:#1a4d8f;font-size:14px">DAILY INSPECTION REPORT</h3>'+itCard.querySelector('.card-body').innerHTML+'</div>';

  // Narrative
  var nt=document.getElementById('nar-text');
  if(nt)bodyHtml+='<div style="margin-bottom:10px;page-break-before:always"><h3 style="color:#1a4d8f;font-size:14px">NARRATIVE</h3><pre style="font-size:16px;line-height:1.8;white-space:pre-wrap;border:1px solid #ccc;padding:12px">'+esc(nt.value)+'</pre></div>';

  // Signatures
  var sigBlock=document.querySelector('.sig-block');
  if(sigBlock)bodyHtml+='<div style="margin-bottom:10px;page-break-before:always">'+sigBlock.outerHTML+'</div>';

  // Footer
  bodyHtml+='<div style="text-align:center;padding:10px 0 0;margin-top:12px;border-top:1px solid #ccc;font-family:monospace">';
  bodyHtml+='<img src="icon-192.png" style="width:28px;height:28px;border-radius:50%;vertical-align:middle"> ';
  bodyHtml+='<span style="font-size:9px;color:#1a4d8f;letter-spacing:1px;font-weight:700">-=pagong=-</span> ';
  bodyHtml+='<span style="font-size:8px;color:#666">JOEY SABENACIO HEREDERO &middot; OPM</span></div>';

  var doc='<!DOCTYPE html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"><title>'+esc(title)+'</title>';
  doc+='<style>body{font-family:Arial,sans-serif;font-size:11px;color:#000;max-width:100%;margin:0.5in;line-height:1.4}h3{font-size:13px;color:#1a4d8f;border-bottom:1px solid #ccc;padding-bottom:4px;margin:12px 0 6px}table{width:100%;border-collapse:collapse;font-size:9px;margin:6px 0}th{background:#e8ecf0;color:#1a4d8f;padding:4px 6px;border:1px solid #999;text-align:left;font-size:8px}td{padding:3px 6px;border:1px solid #ddd;font-size:9px}.stats{display:grid;grid-template-columns:repeat(6,1fr);gap:1px;background:#ccc}.stats div{padding:6px;text-align:center;background:#fff}.stats .n{font-size:18px;font-weight:700;color:#1a4d8f}.stats .l{font-size:7px;color:#666}.photo-card{display:grid;grid-template-columns:1fr 1fr;border:1px solid #ddd;margin:3px 0;page-break-inside:avoid}.photo-side{position:relative;min-height:120px;background:#f0f0f0}.photo-side img{width:100%;height:120px;object-fit:cover}.info-side{padding:4px 8px;font-size:9px}.info-side .fn{font-size:12px;font-weight:700;color:#1a4d8f}.fr{display:grid;grid-template-columns:auto 1fr;gap:1px 4px;font-size:8px}.fk{color:#1a4d8f;font-size:7px}.pb,.pn{display:none}.sig-block{border:1px solid #999;padding:14px;margin-top:10px;page-break-before:always}.sig-block .card-hdr{text-align:center;font-size:12px;color:#1a4d8f;border-bottom:1px solid #999;padding-bottom:6px;margin-bottom:10px}.sig-section{margin-bottom:10px}.sig-label{font-size:10px;color:#1a4d8f;font-weight:700}.sig-line{border-bottom:1px solid #333;height:30px;margin-bottom:4px}.sig-hint{font-size:7px;color:#666}.sig-row-3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin:12px 0}.sig-row-2{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin:12px 0}.sig-col{text-align:center}.sig-col .sig-line{border-bottom:1px solid #333;height:28px;margin-bottom:10px}.sig-col .sig-name{font-size:11px;font-weight:700;color:#000}.sig-col .sig-title{font-size:8px;color:#555}.inspect-tbl{font-size:8px}@page{margin:0.7in;size:A4}@media print{body{margin:0.5in}}</style></head><body>';
  doc+='<div style="text-align:center;margin-bottom:14px"><img src="bpgt-logo.png" style="max-width:140px;max-height:56px"><h1 style="font-size:16px;color:#1a4d8f;letter-spacing:1px;margin:4px 0">'+esc(title)+'</h1><p style="font-size:9px;color:#555">'+esc(loc)+'</p><p style="font-size:8px;color:#999">Period: '+esc(df)+' to '+esc(dt)+' | Unit: '+esc(su)+'</p></div>';
  doc+=bodyHtml;
  doc+='</body></html>';
  ...
  a.download='BPGT_Report_'+su.replace(/[^a-z0-9]/gi,'_')+'_'+(df||'any')+'_'+(dt||'any')+'.doc';
  a.click();
  URL.revokeObjectURL(a.href);
  OPM.toast('Word document downloaded','success');
}
